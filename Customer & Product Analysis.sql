/*
customers table contains sensitive info such as business name, owner's full name, contact information, residency, and credit availability.

employee table contains basic information such as full name, work email, work id and who they report to.
...
orders table contains dates on when order took place, when the order has been shipped, and when it needs to be shipped as well as any comments on the order.

payment table cointains customer numbers with payment amounts when payment was made with the checking number.

productline table group of products with a description of each product. 

product table contains each product with productline categorization and description of each product followed with quantity in stock, also has the buy price and MSRP.
*/
...
-------------------------------------------------------------------------------------------------
/*
 Q2.P1 How Should We Match Marketing and Communication Strategies to Customer Behavior?
 ...
 */
 ...
 -------------------------------------------------------------------------------------------------
 /*
 Q2.P2 Now that we have a query we can begin grouping our customers, and to achieve this I'll be using a CTE to find the 
 TOP 5 VIP Customers. 
 */
